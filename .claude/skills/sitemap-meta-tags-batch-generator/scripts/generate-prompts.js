...
async function main() {
  ...
  const urlList = urls.map((url, index) => {
    ...
    const metaTagsFileName = `url-${index}.json`;
    
    const promptPath = path.join(outputDir, promptFileName);
    const metaTagsPath = path.join(outputDir.replace('/prompts', '/meta-tags'), metaTagsFileName);
    
    const prompt = generatePrompt(url, metaTagsPath, template);
