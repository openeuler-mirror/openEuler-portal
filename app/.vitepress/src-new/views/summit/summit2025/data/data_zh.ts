import bgLight from '../img/banner-pc-light.jpg';
import bgDark from '../img/banner-pc-dark.jpg';
import bgMbLight from '../img/banner-mb-light.jpg';
import bgMbDark from '../img/banner-mb-dark.jpg';

import textLight from '../img/textImg-pc-light.png';
import textDark from '../img/textImg-pc-dark.png';
import textMbLight from '../img/textImg-mb-light.png';
import textMbDark from '../img/textImg-mb-dark.png';

import openEuler from '../img/logo/openEuler_light.png';
import openEulerDark from '../img/logo/openEuler_dark.png';
import huawei from '../img/logo/huawei_light.png';
import huawei_dark from '../img/logo/huawei_dark.png';
import qiling from '../img/logo/qiling_light.png'
import qiling_dark from '../img/logo/qiling_dark.png';
import xinan from '../img/logo/xinan_light.png';
import xinan_dark from '../img/logo/xinan_dark.png';
import ningsi from '../img/logo/ningsi_light.png';
import ningsi_dark from '../img/logo/ningsi_dark.png';
import xfusion from '../img/logo/xfusion_light.png';
import xfusion_dark from '../img/logo/xfusion_dark.png';
import dongfang from '../img/logo/dongfang_light.png';
import dongfang_dark from '../img/logo/dongfang_dark.png';
import ruantong from '../img/logo/ruantong_light.png';
import ruantong_dark from '../img/logo/ruantong_dark.png';
import runhe from '../img/logo/runhe_light.png';
import runhe_dark from '../img/logo/runhe_dark.png';
import baolande from '../img/logo/baolande_light.png';
import baolande_dark from '../img/logo/baolande_dark.png';
import tongxin from '../img/logo/tongxin_light.png';
import tongxin_dark from '../img/logo/tongxin_dark.png';

import xiexiuqi from '../img/guest/xiexiuqi.png';
import luoyukai from '../img/guest/luoyukai.png';
import dukaitian from '../img/guest/dukaitian.png';
import lichenbin from '../img/guest/lichenbin.png';
import tanggeliang from '../img/guest/tanggeliang.png';
import zhutao from '../img/guest/zhutao.png';
import zhangyizheng from '../img/guest/zhangyizheng.png';
import chenxin from '../img/guest/chenxin.png';
import zhuzexu from '../img/guest/zhuzexu.png';
import liyang from '../img/guest/liyang.png';

import dudongdong from '../img/guest/dudongdong.png';
import lujun from '../img/guest/lujun.png';
import zhangtao from '../img/guest/zhangtao.png';
import jianghaoran from '../img/guest/jianghaoran.png';
import duanchenghao from '../img/guest/duanchenghao.png';
import tangyouling from '../img/guest/tangyouling.png';
import songshi from '../img/guest/songshi.png';
import yaoguangzhong from '../img/guest/yaoguangzhong.png';
import yanglu from '../img/guest/yanglu.png';
import zhangrui from '../img/guest/zhangrui.png';
import wangwenkuan from '../img/guest/wangwenkuan.png';
import cryolitia from '../img/guest/cryolitia.png';
import songjiayu from '../img/guest/songjiayu.png';
import renwei from '../img/guest/renwei.png';
import wangjingwei from '../img/guest/wangjingwei.png';
import jinxiaoxu from '../img/guest/jinxiaoxu.png';
import zhangxiao from '../img/guest/zhangxiao.png';
import zhangzehua from '../img/guest/zhangzehua.png';
import sunzhenghua from '../img/guest/sunzhenghua.png';
import zhouqiang from '../img/guest/zhouqiang.png';
import liubingwei from '../img/guest/liubingwei.png';

import caixinqi from '../img/guest/caixinqi.png';
import zengyi from '../img/guest/zengyi.png';
import dingjiahui from '../img/guest/dingjiahui.png';
import feijianqiang from '../img/guest/feijianqiang.png';
import hukuang from '../img/guest/hukuang.png';
import jiangdongping from '../img/guest/jiangdongping.png';
import jinlong from '../img/guest/jinlong.png';
import lichaoyang from '../img/guest/lichaoyang.png';
import lipeng from '../img/guest/lipeng.png';
import liangyi from '../img/guest/liangyi.png';
import wujian from '../img/guest/wujian.png';
import liubaoyu from '../img/guest/liubaoyu.png';
import luweijun from '../img/guest/luweijun.png';
import tanrui from '../img/guest/tanrui.png';
import wanfeng from '../img/guest/wanfeng.png';
import wangdawei from '../img/guest/wangdawei.png';
import wangwei from '../img/guest/wangwei.png';
import wangxiaoyun from '../img/guest/wangxiaoyun.png';
import wangzhiqiang from '../img/guest/wangzhiqiang.png';
import weijiangang from '../img/guest/weijiangang.png';
import wuheng from '../img/guest/wuheng.png';
import wuzhiguo from '../img/guest/wuzhiguo.png';
import xulei from '../img/guest/xulei.png';
import xupeiwen from '../img/guest/xupeiwen.png';
import yangbin from '../img/guest/yangbin.png';
import yunlipeng from '../img/guest/yunlipeng.png';
import zhuowenjie from '../img/guest/zhuowenjie.png';
import zhongmaoheng from '../img/guest/zhongmaoheng.png';
import zhongtao from '../img/guest/zhongtao.png';
import liuxinliang from '../img/guest/liuxinliang.png';

import chengxiaoming from '../img/guest/chengxiaoming.png';
import niguangnan from '../img/guest/niguangnan.png';
import penghonghua from '../img/guest/penghonghua.png';
import xiongwei from '../img/guest/xiongwei.png';
import yangchaobin from '../img/guest/yangchaobin.png';
import yangmingming from '../img/guest/yangmingming.png';
import zengwenbin from '../img/guest/zengwenbin.png';
import shaoguanglu from '../img/guest/shaoguanglu.png';
import duhai from '../img/guest/duhai.png';

import andrew from '../img/guest/andrew.png';
import chenyouxuan from '../img/guest/chenyouxuan.png';
import tangxiaolei from '../img/guest/tangxiaolei.png';
import gaolijiang from '../img/guest/gaolijiang.png';
import grant from '../img/guest/grant.png';
import huchunming from '../img/guest/huchunming.png';
import huxinwei from '../img/guest/huxinwei.png';
import linzhenfu from '../img/guest/linzhenfu.png';
import yeqinglong from '../img/guest/yeqinglong.png';

import andy from '../img/guest/andy.png';
import heliyun from '../img/guest/heliyun.png';
import liping from '../img/guest/liping.png';
import wanglin from '../img/guest/wanglin.png';
import wangyucheng from '../img/guest/wangyucheng.png';
import wuchenggang from '../img/guest/wuchenggang.png';
import xinxiaohui from '../img/guest/xinxiaohui.png';
import zhangchi from '../img/guest/zhangchi.png';
import fengyan from '../img/guest/fengyan.png';
import hanyuankun from '../img/guest/hanyuankun.png';
import huangjun from '../img/guest/huangjun.png';
import lijingjing from '../img/guest/lijingjing.png';
import liyuzhen from '../img/guest/liyuzhen.png';
import sunyanan from '../img/guest/sunyanan.png';
import zhegnchenhui from '../img/guest/zhegnchenhui.png';

export default {
  banner: {
    bg: {
      light: bgLight,
      dark: bgDark,
    },
    bgMb: {
      light: bgMbLight,
      dark: bgMbDark,
    },
    textImg: {
      light: textLight,
      dark: textDark,
    },
    textImgMb: {
      light: textMbLight,
      dark: textMbDark,
    },
    signUpTitle: '立即报名',
    watchReplay: '观看回放',
    replayLink: '/zh/interaction/summit-list/summit2025/#live',
    link: {
      pc: 'https://openatomcon.openatom.cn/registration/?code=Nhvt&activityNo=HD20251013162127',
      mo: 'https://openatomcon.openatom.cn/registration_mobile/?code=Nhvt&activityNo=HD20251013162127',
    },
  },
  introduce: {
    desc: [
      'openEuler 开源六载，已全面支持多样性算力，覆盖互联网、金融、运营商等多领域核心应用场景，市场份额稳居首位。面向下一个五年，openEuler 将全面聚焦AI，践行AI For OS、OS For AI的理念，开启操作系统变革的全新篇章。',
      '2025年11月14日-15日，我们将在北京中关村国际创新中心举办 操作系统大会&openEuler Summit 2025，发布openEuler 在AI领域的重要技术版本，并邀请全球伙伴发布重磅产品，以技术创新与全球化实践共同开启 openEuler 下一个五年的全新篇章。'
    ],
    tips: '我们诚挚地邀请您莅临大会现场，在这里您将可以：',
    list: [
      '见证全新一代操作系统的诞生与发布，共同探索未来操作系统发展路径',
      '见证全球合作伙伴的重磅产品发布，揭开全球化发展的序章',
      '参与30+场主题演讲、论坛圆桌，与全球专家与开发者交流，聆听前沿观点和落地案例',
      '参观1000㎡交流区，体验最新技术创新并动手实践，感受伙伴联创成果的全景演绎'
    ],
    slogan: [
      '让我们相约北京，在思维碰撞与技术交流中，为全球开源新生态的建设注入核心力量。'
    ],
  },
  live: {
    title: '活动直播',
    list: [
      {
        time: '11月14日',
        lives: [
          {
            liveId: 15926,
            liveTestId: 15912,
            id: 0,
            name: '操作系统大会 主题演讲',
          },
          {
            liveId: 15928,
            liveTestId: 15914,
            name: '科教创新',
          },
          {
            liveId: 15929,
            liveTestId: 15915,
            name: '智能制造产业',
          },
          {
            liveId: 15930,
            liveTestId: 15916,
            name: '麒麟软件',
          },
          {
            liveId: 15931,
            liveTestId: 15917,
            name: '麒麟信安',
          },
        ]
      },
      {
        time: '11月15日',
        lives: [
          {
            liveId: 15927,
            liveTestId: 15913,
            id: 0,
            name: 'openEuler Summit 主题演讲',
          },
          {
            liveId: 15932,
            liveTestId: 15918,
            name: '内核&全场景',
          },
          {
            liveId: 15933,
            liveTestId: 15919,
            name: '安全与机密计算',
          },
          {
            liveId: 15934,
            liveTestId: 15920,
            name: 'AI',
          },
          {
            liveId: 15935,
            liveTestId: 15921,
            name: '云原生',
          },
          {
            liveId: 15936,
            liveTestId: 15922,
            name: '灵衢',
          },
          {
            liveId: 15937,
            liveTestId: 15923,
            name: '嵌入式&具身智能',
          },
          {
            liveId: 15938,
            liveTestId: 15924,
            name: '开发者使能与工具',
          },
          {
            liveId: 15939,
            liveTestId: 15925,
            name: 'RISC-V',
          },
        ],
      }
    ]
  },
  agent: {
    title: '活动日程',
    tips: '具体内容，敬请期待',
    text: '查看详情',
    list: [
      {
        date: "14",
        children: [
          {
            title: '主论坛',
            children: [
              {
                time: '09:30 - 09:55',
                content: [
                  {
                    title: '大会致辞',
                    guest: [
                      {
                        name: '倪光南',
                        post: '中国工程院院士',
                      },
                      {
                        name: '程晓明',
                        post: '开放原子开源基金会理事长',
                      },
                      {
                        name: '杨超斌',
                        post: '华为公司董事、ICT BG CEO',
                      },
                      {
                        name: '邵广禄',
                        post: '中国电信集团科技委主任、中国电信集团原总经理',
                      },
                    ]
                  },
                ]
              },
              {
                time: '09:55 - 11:05',
                content: [
                  {
                    title: '智跃无界，openEuler引领AI时代操作系统',
                    name: '熊伟',
                    post: '开放原子开源基金会 openEuler委员会主席',
                  },
                  {
                    title: '智启新程 生态共荣：CTyunOS 4.0 开启智能云网操作系统新时代',
                    name: '	蒋彪',
                    post: '中国电信集团首席专家',
                  },
                  {
                    title: 'AI算力的极限挑战：BaiduLinux支持超大规模集群基础设施建设实践',
                    name: '杜海',
                    post: '百度智能云混合云部总经理',
                  },
                  {
                    title: 'MTOS & openEuler 联创共赢、生态共振 ，探索超节点优势场景',
                    name: '曾文斌',
                    post: '美团操作系统总监',
                  },
                  {
                    title: '菲尼克斯携手openEuler，加速OT产业IT化转型',
                    name: '杨明明',
                    post: '菲尼克斯中国公司副总裁',
                  },
                ]
              },
              {
                time: '11:05 - 11:15',
                content: [
                  {
                    title: '仪式：操作系统实践单位致谢暨2025年度openEuler领先商业实践',
                    name: '领先商业实践用户代表',
                    post: '',
                  },
                  {
                    title: '仪式：openEuler全球化先锋伙伴致谢',
                    name: '伙伴代表',
                    post: '',
                  },
                ]
              },
            ],
            tab: []
          },
          {
            title: '分论坛',
            children: [],
            tab: [
              {
                label: '麒麟软件',
                time: '13:30 - 15:45',
                address: '',
                children: [
                  {
                    time: '13:30-13:40',
                    content: [
                      {
                        title: '开场致辞',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:40-13:50',
                    content: [
                      {
                        title: '开场致辞',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:50-14:05',
                    content: [
                      {
                        title: '拥抱开源・AI 筑基：银河麒麟服务器 OS 一体化方案',
                        name: '侯健',
                        post: '麒麟软件有限公司服务器研发中心副总经理',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: '人工智能与档案管理融合',
                        name: '夏光军',
                        post: '北京量子伟业信息技术股份有限公司副总经理',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:35',
                    content: [
                      {
                        title: '基于星普大模型赋能数智校园建设思考与实践分享',
                        name: '张卫兵',
                        post: '新开普AI研究院解决方案总监',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: '筑算力长城，强自主创新——异构算力底座赋能行业创新',
                        name: '吴跃搏',
                        post: '中电长城科技有限公司解决方案总监',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: '基于自主创新建设，端边云办公生产力新思路',
                        name: '贾安亚',
                        post: '上海商汤智能科技有限公司高级产品总监',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: '超云私域部署AI模型一体机实践分享',
                        name: '刘岩岩',
                        post: '超云数字技术集团有限公司首席架构师',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:35',
                    content: [
                      {
                        title: 'AI-COP 共创生态智能化新范式',
                        name: '关俊君',
                        post: '北京致远互联软件股份有限公司生态合作总监',
                      }
                    ]
                  },
                  {
                    time: '15:35-15:45',
                    content: [
                      {
                        title: '幸运抽奖',
                        name: '',
                        post: '',
                      }
                    ]
                  }
                ],
              },
              {
                label: '麒麟信安',
                time: '14:30 - 17:00',
                address: '',
                children: [
                  {
                    time: '14:30-14:35',
                    content: [
                      {
                        title: '开场致辞',
                        name: '熊伟',
                        post: 'openEuler社区主席',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:45',
                    content: [
                      {
                        title: '开场致辞',
                        name: '杨涛',
                        post: '麒麟信安董事长',
                      }
                    ]
                  },
                  {
                    time: '14:45-15:05',
                    content: [
                      {
                        title: '麒麟信安操作系统V6发布',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:25',
                    content: [
                      {
                        title: '麒麟信安机器人操作系统发布',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:25-15:45',
                    content: [
                      {
                        title: '麒麟信安智能云新品发布',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:45-16:00',
                    content: [
                      {
                        title: '智算应用实践分享',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:00-16:15',
                    content: [
                      {
                        title: '工业领域应用实践分享',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:15-16:30',
                    content: [
                      {
                        title: '电力领域应用实践分享',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:30-16:45',
                    content: [
                      {
                        title: '金融领域应用实践分享',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:45-17:00',
                    content: [
                      {
                        title: '内网虚拟化应用实践分享',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  }
                ],
              },
              {
                label: '互联网',
                time: '14:00 - 17:00',
                address: '',
                children: [
                  {
                    time: '14:00-14:05',
                    content: [
                      {
                        title: '欢迎致辞',
                        name: '特邀嘉宾',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '14:05-14:10',
                    content: [
                      {
                        title: '持续开源开放，携手互联网深耕openEuler社区',
                        name: '特邀嘉宾',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '14:10-15:10',
                    content: [
                      {
                        title: '互联网灵衢超节点实践分享和价值场景研讨',
                        name: '特邀嘉宾',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '15:10-15:50',
                    content: [
                      {
                        title: '互联网智算集群实践分享和研讨',
                        name: '特邀嘉宾',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '15:50-16:30',
                    content: [
                      {
                        title: '互联网众核高密场景实践分享和研讨',
                        name: '特邀嘉宾',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '16:30-17:00',
                    content: [
                      {
                        title: '互联网开源欧拉+鲲鹏实践分享和研讨',
                        name: '特邀嘉宾',
                        post: '',
                      },
                    ]
                  },
                ]
              },
              {
                label: '全球生态',
                time: '13:30 - 15:00',
                address: '',
                children: [
                  {
                    time: '13:30-13:35',
                    content: [
                      {
                        title: '开场致辞',
                        name: '付海巍',
                        post: '开放原子开源基金会业务发展部部长',
                      }
                    ]
                  },
                  {
                    time: '13:35-13:50',
                    content: [
                      {
                        title: '硬件厂商致辞',
                        guest: [
                          {
                            name: 'Andrew Wafaa',
                            post: 'ARM公司Fellow与软件社区高级总监',
                          },
                          {
                            name: '刘秉伟',
                            post: '英特尔（中国）软件技术事业部高级总监',
                          },
                          {
                            name: '王文宽',
                            post: '超威半导体产品（中国）有限公司 AMD Linux内核专家',
                          },
                        ]
                      }
                    ]
                  },
                  {
                    time: '13:50-14:00',
                    content: [
                      {
                        title: '开源基础软件与AI技术创新路线',
                        name: '熊伟',
                        post: 'openEuler委员会主席',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:15',
                    content: [
                      {
                        title: '运营商智能化实践分享',
                        name: '杨雨超',
                        post: '天翼云国际业务事业部高级技术专家',
                      }
                    ]
                  },
                  {
                    time: '14:15-14:30',
                    content: [
                      {
                        title: '互联网智能化实践分享',
                        name: '张泽华',
                        post: '京东算法总监',
                      }
                    ]
                  },
                  {
                    time: '14:30-14:40',
                    content: [
                      {
                        title: 'AI训推应用案例分享',
                        name: '张骁',
                        post: '讯飞星火AI工程院智算平台总监',
                      }
                    ]
                  },
                  {
                    time: '14:40-14:50',
                    content: [
                      {
                        title: 'ERP应用案例分享',
                        name: '靳晓旭',
                        post: '用友中企出海业务总监',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:00',
                    content: [
                      {
                        title: 'openEuler全球系统化服务',
                        name: '周强',
                        post: '风河高级交付总监',
                      }
                    ]
                  },
                ],
              },
              {
                label: '科教创新',
                time: '13:30 - 16:30',
                address: '',
                children: [
                  {
                    time: '13：30-13：40',
                    content: [
                      {
                        title: '开场致辞',
                        guest: [
                          {
                            name: '胡欣蔚',
                            post: 'openEuler社区技术委员会主席',
                          },
                          {
                            name: '胡春明',
                            post: '北京航空航天大学软件学院院长',
                          }
                        ]
                      }
                    ]
                  },
                  {
                    time: '13:40-15:00',
                    content: [
                      {
                        title: '《基于高速互联总线的异构池化系统关键技术研究》',
                        name: '赵新奎',
                        post: '浙江大学软件学院百人计划研究员',
                      },
                      {
                        title: '《面向推荐大模型的参数存储系统研究》',
                        name: '谢旻晖',
                        post: '中国人民大学信息学院计算机系讲师',
                      },
                      {
                        title: '《人工智能端到端的复杂攻击检测技术研究》',
                        name: '李锭',
                        post: '北京大学助理教授',
                      },
                      {
                        title: '《异构融合的AI Infra系统软件资源管理与可靠性保障》',
                        name: '杨任宇',
                        post: '北京航空航天大学副教授',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:05',
                    content: [
                      {
                        title: '开源之夏优秀项目',
                        name: '颁奖仪式',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: '优秀学生分享',
                        name: '特邀嘉宾',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:50',
                    content: [
                      {
                        title: '《开源驱动的教育实践-从校源行到人才评价》',
                        name: '韩江',
                        post: '开放原子开源基金会教育培训部负责人',
                      },
                      {
                        title: '《系统软件课程建设与产学合作》',
                        name: '张莉',
                        post: '北京航空航天大学教授',
                      }
                    ]
                  },
                  {
                    time: '15:50-16:30',
                    content: [
                      {
                        title: '《聚力开源连接学术界，共话产学研生态赢未来》',
                        name: '胡春明',
                        post: '',
                      }
                    ]
                  }
                ],
              },
              {
                label: '智能制造',
                time: '13:30 - 16:20',
                address: '',
                children: [
                  {
                    time: '13:30-13:40',
                    content: [
                      {
                        title: '欢迎致辞',
                        name: '刘程',
                        post: 'openEuler产品规划专家',
                      }
                    ]
                  },
                  {
                    time: '13:40-14:00',
                    content: [
                      {
                        title: 'PLCnext Technology赋能智能制造的应用',
                        name: '杨露',
                        post: '菲尼克斯PLCnext Software Application总监',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:20',
                    content: [
                      {
                        title: '面向智能自动化的工业控制底座技术',
                        name: '王志强',
                        post: 'CODESYS软件集团(中国)市场负责人',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:40',
                    content: [
                      {
                        title: '基于开源欧拉面向大科学装置黑灯实验室（工厂）场景的统一操作系统研发与实践',
                        name: '卢俊',
                        post: '中国科学院广州生物医药与健康研究院 谱系工程技术中心副主任',
                      }
                    ]
                  },
                  {
                    time: '14:40-15:00',
                    content: [
                      {
                        title: '支撑高端数控系统应用场景的工业实时操作系统需求演化趋势',
                        name: '宋师',
                        post: '广州数控设备有限公司总工程师',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:20',
                    content: [
                      {
                        title: '拓斯达在具身智能机器人领域的场景探索与实践',
                        name: '曾逸',
                        post: '广东拓斯达科技股份有限公司机器人市场总监',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:40',
                    content: [
                      {
                        title: 'openEuler Embedded在智慧交通工控场景的应用',
                        name: '王伟',
                        post: '成都菁蓉联创科技有限公司无人装备事业部部长',
                      }
                    ]
                  },
                  {
                    time: '15:40-16:00',
                    content: [
                      {
                        title: '软件赋能工业智能化',
                        name: '姚广忠',
                        post: '河南昆仑技术有限公司技术专家',
                      }
                    ]
                  },
                  {
                    time: '16:00-16:20',
                    content: [
                      {
                        title: 'openEuler 嵌入式适配昇腾310B解锁边缘 AI 高效部署新范式',
                        name: '张涛',
                        post: '江苏润和软件股份有限公司嵌入式部门经理',
                      }
                    ]
                  }
                ],
              },
            ]
          }
        ]
      },
      {
        date: "15",
        children: [
          {
            title: '主论坛',
            children: [
              {
                time: '09:30-09:35',
                content: [
                  {
                    title: '大会致辞',
                    name: '彭红华',
                    post: '华为ICT战略与业务发展部总裁',
                  }
                ]
              },
              {
                time: '09:35 - 10:05',
                content: [
                  {
                    titles: ['openEuler社区发展主题演讲：', '长风破浪会有时，直挂云帆济沧海'],
                    guest: [
                      {
                        name: '熊伟',
                        post: '开放原子开源基金会 openEuler委员会主席',
                      },
                      {
                        name: '唐晓蕾',
                        post: 'AMD公司全球副总裁',
                      },
                      {
                        name: 'Andrew Wafaa',
                        post: 'PyTorch Board 成员',
                      },
                    ]
                  },
                  {
                    title: '仪式：openEuler 社区学术与教育委员会成立',
                    guest: [
                      {
                        name: '特邀嘉宾',
                        post: '',
                      },
                    ]
                  }
                ]
              },
              {
                time: '10:05 - 10:40',
                content: [
                  {
                    title: 'openEuler技术前沿与展望',
                    guest: [
                      {
                        name: '胡欣蔚',
                        post: 'openEuler技术委员会主席',
                      },
                      {
                        name: '胡春明',
                        post: '北京航空航天大学软件学院院长',
                      },
                      {
                        name: 'Grant Likely',
                        post: 'CTO at Linaro Ltd. ',
                      },
                    ]
                  },
                ]
              },
              {
                time: '10:40 - 11:00',
                content: [
                  {
                    title: '构建中国邮政安全可信的云原生镜像体系实践分享',
                    name: '高立江',
                    post: '中邮信息科技（北京）有限公司云原生平台负责人',
                  },
                  {
                    title: '基于麒麟信安的广汽自主创新高性能计算建设成果',
                    guest: [
                      {
                        name: '叶青龙',
                        post: '麒麟信安(广东)科技有限公司研发总监',
                      },
                      {
                        name: '陈友宣',
                        post: '广州汽车集团股份有限公司高性能计算负责人'
                      }
                    ]
                  },
                ]
              },
              {
                time: '11:00 - 11:15',
                content: [
                  {
                    titles: ['仪式：openEuler 2025年度突出贡献单位', 'openEuler 2025年度项目之星', 'openEuler 2025年度贡献之星'],
                    class: 'desc-indent',
                    name: '特邀嘉宾',
                    post: '',
                  },
                ]
              },
            ],
            tab: []
          },
          {
            title: '分论坛',
            children: [],
            tab: [
              {
                label: '嵌入式&具身智能',
                time: '13:00 - 16:15',
                address: '',
                children: [
                  {
                    time: '13:00-13:15',
                    content: [
                      {
                        title: '开放·融合·智能：vPLCnext基于openEuler的探索与产业实践',
                        name: '徐佩文',
                        post: '南京菲尼克斯电气有限公司',
                      }
                    ]
                  },
                  {
                    time: '13:15-13:30',
                    content: [
                      {
                        title: '基于openEuler的人工智能+星载领域天基底座构建',
                        name: '王大维',
                        post: '成都菁蓉联创科技有限公司',
                      }
                    ]
                  },
                  {
                    time: '13:30-13:45',
                    content: [
                      {
                        title: 'openEuler在航天领域的应用实践与探索',
                        name: '蔡鑫奇',
                        post: '北京航天微系统与信息技术研究所',
                      }
                    ]
                  },
                  {
                    time: '13:45-13:50',
                    content: [
                      {
                        title: '航天鸿域OS 1.0发布',
                        name: '',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:50-14:05',
                    content: [
                      {
                        title: '面向现代机器人的编程看框架AimRT',
                        name: '胡旷',
                        post: '智元机器人',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: 'AGIROS机器人共性算法框架',
                        name: '李鹏',
                        post: '中国科学院软件研究所',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:35',
                    content: [
                      {
                        title: '基于欧拉嵌入式的行业具身智能解决方案及应用实践',
                        name: '吴健',
                        post: '鼎桥技术有限公司 机器人解决方案首席架构师',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: '新一代具身智能电子架构',
                        name: '谭睿',
                        post: '北京光亚鸿道操作系统有限公司',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: '超聚变具身智能操作系统探索与实践',
                        name: '徐磊',
                        post: '超聚变数字技术有限公司',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: '基于全国产化技术的自主移动机器人设计方案',
                        name: '江东平',
                        post: '南京启诺信息技术有限公司',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:35',
                    content: [
                      {
                        title: '国创嵌入式基础软件QSemOS+IntarkDB创新实践',
                        name: '刘保玉',
                        post: '粤港澳大湾区国家技术创新中心',
                      }
                    ]
                  },
                  {
                    time: '15:35-15:50',
                    content: [
                      {
                        title: '基于声明式编排技术的跨节点多系统混合部署设计与实现',
                        name: '魏建刚',
                        post: '江苏润和软件股份有限公司',
                      }
                    ]
                  },
                  {
                    time: '15:50-16:00',
                    content: [
                      {
                        title: '中场休息',
                        name: '',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:00-16:15',
                    content: [
                      {
                        title: 'ROS SIG发展规划与机器人应用实践',
                        name: '王晓云',
                        post: '中国科学院软件研究所',
                      }
                    ]
                  }
                ],
              },
              {
                label: '开发者使能与工具',
                time: '13:00 - 15:50',
                address: '',
                children: [
                  {
                    time: '13:00 - 13:05',
                    content: [
                      {
                        title: '开场致辞',
                        name: '刘飞',
                        post: 'openEuler PE',
                      }
                    ]
                  },
                  {
                    time: '13:05-13:20',
                    content: [
                      {
                        title: 'OPEA、openHPC在openEuler上原生生态进展',
                        name: '鲁卫军',
                        post: '华为技术有限公司 高级工程师',
                      }
                    ]
                  },
                  {
                    time: '13:20-13:35',
                    content: [
                      {
                        title: 'DevStation-面向开发者的AI智能化开发者工作站',
                        name: '袁礼鹏',
                        post: '华为技术有限公司 高级工程师',
                      }
                    ]
                  },
                  {
                    time: '13:35-13:50',
                    content: [
                      {
                        title: '基于openeuler的操作系统轻量化定制构建和部署',
                        name: '廖卫',
                        post: '麒麟信安 研发工程师',
                      }
                    ]
                  },
                  {
                    time: '13:50-14:05',
                    content: [
                      {
                        title: '基于libcareplus的热补丁工具增强开发及支持多架构，多语言能力',
                        name: '吴衡',
                        post: '天翼云 研发专家',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: 'openEuler创新版本改善贡献改进总结',
                        name: 'Funda Wang',
                        post: '个人贡献者',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:35',
                    content: [
                      {
                        title: 'AI赋能的软件工程平台',
                        name: '郭纪岩',
                        post: '北京凝思产品研发总监',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: 'C转Rust的AI自动化方法',
                        name: '张海东',
                        post: '统信软件技术研发经理',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: 'AI工具的所见即所得：基于文档的开发模式',
                        name: '丁嘉辉',
                        post: '华为技术有限公司 高级工程师',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: 'AI大模型在基础设施的实践与应用',
                        name: '翟文杰',
                        post: '华为技术有限公司 高级工程师',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:35',
                    content: [
                      {
                        title: 'PortGPT: 基于大模型的自动化补丁后向迁移研究',
                        name: '李朝阳',
                        post: '华中科技大学',
                      }
                    ]
                  },
                  {
                    time: '15:35-15:50',
                    content: [
                      {
                        title: '轻量浏览器引擎LiteView',
                        name: '许景民',
                        post: '中国科学院软件研究所LiteView内核架构师',
                      }
                    ]
                  }
                ],
              },
              {
                label: '云原生',
                time: '13:00 - 15:35',
                address: '',
                children: [
                  {
                    time: '13:00-13:10',
                    content: [
                      {
                        title: '开场致辞',
                        name: '胡欣蔚',
                        post: 'openEuler社区技术委员会主席',
                      }
                    ]
                  },
                  {
                    time: '13:10-13:30',
                    content: [
                      {
                        title: 'openYuanrong Serverless分布式计算引擎技术和实践',
                        name: '梁义',
                        post: '华为技术有限公司 通用Serverless首席专家',
                      }
                    ]
                  },
                  {
                    time: '13:30-13:45',
                    content: [
                      {
                        title: 'Apache Doris基于Kunpeng的性能优化实践',
                        name: '吴治国',
                        post: 'Linaro 工程师',
                      }
                    ]
                  },
                  {
                    time: '13:45-13:55',
                    content: [
                      {
                        title: 'Agent沙箱对传统云原生架构的挑战',
                        name: '费建强',
                        post: '华为技术有限公司 云原生专家',
                      }
                    ]
                  },
                  {
                    time: '13:55-14:05',
                    content: [
                      {
                        title: 'Agentic AI超容器Conch技术介绍',
                        name: '钟涛',
                        post: '华为技术有限公司 工程师',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: '云原生网络拓扑实时可视化和网络运维实践',
                        name: '万峰',
                        post: '麒麟软件 高级软件工程师',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:40',
                    content: [
                      {
                        title: '通过NPU虚拟化解决方案提升AI服务器资源利用效率',
                        name: '杨彬',
                        post: '软通动力信息技术（集团）股份有限公司 开源欧拉研究中心主任',
                      }
                    ]
                  },
                  {
                    time: '14:40-15:00',
                    content: [
                      {
                        title: 'AI Agent赋能云原生基础设施',
                        name: '张盈辉',
                        post: '中国移动信息技术中心 云原生架构师',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:15',
                    content: [
                      {
                        title: '基于CTyunOS的云原生不可变基础设施实践',
                        name: '刘敏',
                        post: '天翼云科技有限公司',
                      }
                    ]
                  },
                  {
                    time: '15:15-15:35',
                    content: [
                      {
                        title: '大数据统一加速底座OmniRuntime开源及实践',
                        name: '尹伟杰',
                        post: '华为技术有限公司 大数据专家',
                      }
                    ]
                  }
                ],
              },
              {
                label: '灵衢',
                time: '13:00 - 15:40',
                address: '',
                children: [
                  {
                    time: '13:00-13:05',
                    content: [
                      {
                        title: '开场致辞',
                        name: '朱照生',
                        post: '华为集群计算总经理',
                      }
                    ]
                  },
                  {
                    time: '13:05-13:25',
                    content: [
                      {
                        title: '超节点和关键应用场景',
                        name: '张健',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:25-13:45',
                    content: [
                      {
                        title: 'CXL在计算机体系重构中的位置、作用和价值',
                        name: '宋家雨',
                        post: 'CXL技术应用俱乐部秘书长',
                      }
                    ]
                  },
                  {
                    time: '13:45-14:00',
                    content: [
                      {
                        title: '灵衢总线技术和软件参考设计',
                        name: '牛涛',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:15',
                    content: [
                      {
                        title: '灵衢内存池化关键技术和应用',
                        name: '高超',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '14:15-14:35',
                    content: [
                      {
                        title: '灵衢通信关键技术和应用',
                        name: '李力军',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: '灵衢设备虚拟化关键技术和应用',
                        name: '叶镖翔',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: '超节点可靠性关键技术',
                        name: '邓广兴',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:25',
                    content: [
                      {
                        title: '灵衢系统高阶服务关键技术和应用',
                        name: '姚弋宇',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:25-15:40',
                    content: [
                      {
                        title: '灵衢超节点编程编译关键技术',
                        name: '杨扬',
                        post: '',
                      }
                    ]
                  },
                ],
              },
              {
                label: 'AI',
                time: '13:00 - 16:00',
                address: '',
                children: [
                  {
                    time: '13:00-13:15',
                    content: [
                      {
                        title: '基于openEuler intelligence组建一支OS开发团队',
                        name: '孙政华',
                        post: '深信服科技有限公司 中央平台部研发主管',
                      }
                    ]
                  },
                  {
                    time: '13:15-12:30',
                    content: [
                      {
                        title: '《破局“医疗孤岛”：基于openEuler的新一代CDSS智能引擎实践》',
                        name: '冯焱',
                        post: '北京惠每云科技有限公司 高级后端工程师',
                      }
                    ]
                  },
                  {
                    time: '13:30-13:45',
                    content: [
                      {
                        title: '基于大模型的新一代核心交易信创系统智能调优探索',
                        name: '黄俊',
                        post: '招商证券股份有限公司 核心系统架构师',
                      }
                    ]
                  },
                  {
                    time: '13:45-14:00',
                    content: [
                      {
                        title: 'JuiceFS MLPerf Storage v2.0 性能测评与调优',
                        name: '刘新良',
                        post: 'Linaro Engineer',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:15',
                    content: [
                      {
                        title: '宝德基于intelligence boom推理优化及智能应用的实践分享',
                        name: '金龙',
                        post: '宝德计算机系统股份有限公司 解决方案总监',
                      }
                    ]
                  },
                  {
                    time: '14:15-14:30',
                    content: [
                      {
                        title: '通算/智算大模型推理加速协同应用与落地',
                        name: '李宇振',
                        post: '华为技术有限公司 AI工程师',
                      }
                    ]
                  },
                  {
                    time: '14:30-14:45',
                    content: [
                      {
                        title: 'AI赋能 - 智能运维平台',
                        name: '钟茂恒',
                        post: '北京凝思软件股份有限公司 研发经理',
                      }
                    ]
                  },
                  {
                    time: '14:45-15:00',
                    content: [
                      {
                        title: '东方通中间件智能体',
                        name: '李静静',
                        post: '东方通科技有限公司 产品经理',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:15',
                    content: [
                      {
                        title: '昇腾亲和的大EP通信加速库技术与生态介绍',
                        name: '韩远坤',
                        post: '华为技术有限公司 华为AI通信专家',
                      }
                    ]
                  },
                  {
                    time: '15:15-15:30',
                    content: [
                      {
                        title: '昆仑技术基于昇腾软FP8的极致调优',
                        name: '孙亚楠',
                        post: '河南昆仑技术有限公司 高级工程师',
                      }
                    ]
                  },
                  {
                    time: '15:30-15:45',
                    content: [
                      {
                        title: 'ANNC：基于XLA的深度学习图编译器在搜推场景的探索与应用',
                        name: '郑晨卉',
                        post: '华为技术有限公司 软件工程师',
                      }
                    ]
                  },
                  {
                    time: '15:45-16:00',
                    content: [
                      {
                        title: 'CTyunOS智能安全中心',
                        name: '吴开顺',
                        post: '天翼云科技有限公司 后端开发工程师',
                      }
                    ]
                  }
                ],
              },
              {
                label: '内核+全场景',
                time: '13:00 - 15:30',
                address: '',
                children: [
                  {
                    time: '13:00-13:15',
                    content: [
                      {
                        title: 'openEuler 内核社区发展介绍',
                        name: '谢秀奇',
                        post: 'openEuler Kernel SIG Maintainer',
                      }
                    ]
                  },
                  {
                    time: '13:15-13:30',
                    content: [
                      {
                        title: 'How to Copy Memory? Coordinated Asynchronous Copy as a First-Class OS Service',
                        name: '杜冬冬',
                        post: '上海交通大学 助理教授',
                      }
                    ]
                  },
                  {
                    time: '13:30-13:45',
                    content: [
                      {
                        title: '午间休息',
                        name: '',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:45-14:00',
                    content: [
                      {
                        title: 'AMD P-State：性能与能效的平衡',
                        name: '王文宽',
                        post: '超威半导体产品（中国）有限公司AMD linux 内核专家',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:15',
                    content: [
                      {
                        title: 'Power for performance on Intel platforms',
                        name: '张锐',
                        post: '英特尔 软件工程师',
                      }
                    ]
                  },
                  {
                    time: '14:15-14:30',
                    content: [
                      {
                        title: '可编程IO调度器设计',
                        name: '程垲涛',
                        post: '麒麟软件 内核工程师',
                      }
                    ]
                  },
                  {
                    time: '14:30-14:45',
                    content: [
                      {
                        title: '基于LoongArch 的trampoline和kexec_file功能开发',
                        name: '姜浩然',
                        post: '麒麟软件 研发工程师',
                      }
                    ]
                  },
                  {
                    time: '14:45-15:00',
                    content: [
                      {
                        title: '基于eBPF的多线程系统信息采集优化',
                        name: '李子遥',
                        post: '统信软件 资深研发工程师',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:15',
                    content: [
                      {
                        title: '基于ebpf的内存-进程-IO-文件缓存观测工具',
                        name: '宋振',
                        post: '麒麟信安 资深内核工程师',
                      }
                    ]
                  },
                  {
                    time: '15:15-15:30',
                    content: [
                      {
                        title: 'UACCE加速器框架使能用户态创新',
                        name: '冯成文',
                        post: '华为技术有限公司 工程师',
                      }
                    ]
                  },
                ],
              },
              {
                label: '安全与机密计算',
                time: '13:00 - 13:05',
                address: '',
                children: [
                  {
                    time: '13:00-13:05',
                    content: [
                      {
                        title: '开场致辞',
                        name: '朱健伟',
                        post: 'openEuler安全委员会主席',
                      }
                    ]
                  },
                  {
                    time: '13:05-13:20',
                    content: [
                      {
                        title: '安全增强复式内核 HAOC 3.0 特性介绍与 4.0 规划',
                        name: '武成岗',
                        post: '中国科学院计算技术研究所，研究员',
                      }
                    ]
                  },
                  {
                    time: '13:20-13:35',
                    content: [
                      {
                        title: '与Arm CCA共铸开源机密计算新时代',
                        name: 'Kaly Xin',
                        post: 'Linaro，软件工程总监',
                      }
                    ]
                  },
                  {
                    time: '13:35-13:50',
                    content: [
                      {
                        title: '基于openEuler的AI安全全栈技术及应用实践',
                        name: '程琨',
                        post: 'openEuler机密计算SIG， Maintainer',
                      }
                    ]
                  },
                  {
                    time: '13:50-14:05',
                    content: [
                      {
                        title: '机密计算应用实践：动态可信证明与机密AI',
                        name: 'Andy',
                        post: '京东，安全工程师',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: 'AI辅助CVE漏洞修复和冲突处理',
                        name: '李萍',
                        post: '华为技术有限公司，高级软件工程师',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:35',
                    content: [
                      {
                        title: 'AI助力开源供应链风险管控与合规审查',
                        name: '王麟',
                        post: '联通数字科技有限公司，高级研发专家',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: '基于鲲鹏机密计算的密码服务平台解决方案',
                        name: '张驰',
                        post: '三未信安科技股份有限公司，技术总监',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: '基于openEuler的系统安全加固方案',
                        name: '王宇诚',
                        post: '湖南麒麟信安科技股份有限公司，研发工程师',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: '基于 SBOM 的供应链安全运营-自动化、可视化与欧拉生态适配实践',
                        name: '贺礼云',
                        post: '凝思软件成都研发中心，副总经理',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:35',
                    content: [
                      {
                        title: '面向TEE的数据库性能优化技术研究',
                        name: '雷卓祎',
                        post: '兰州大学，硕士研究生',
                      }
                    ]
                  },
                ],
              },
              {
                label: 'RISC-V',
                time: '13:00 - 15:05',
                address: '',
                children: [
                  {
                    time: '13:00-13:05',
                    content: [
                      {
                        title: '开场致辞',
                        name: '王经纬',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:05-13:25',
                    content: [
                      {
                        title: 'RISC-V 用户态组件性能优化',
                        name: '兰静',
                        post: 'ZTE-中兴通讯 性能优化工程师',
                      }
                    ]
                  },
                  {
                    time: '13:25-13:45',
                    content: [
                      {
                        title: '进迭时空RISC-V AI CPU 的市场落地探索',
                        name: '陈俭东',
                        post: '进迭时空（杭州）科技有限公司 产品总监',
                      }
                    ]
                  },
                  {
                    time: '13:45-14:05',
                    content: [
                      {
                        title: 'RISC-V PMU 扩展与性能剖析方法的生态演进挑战',
                        name: '邓天迈',
                        post: '超睿科技 资深软件工程师',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:25',
                    content: [
                      {
                        title: '玄铁 C930 虚拟化和 IOMMU 在openEuler 上的开源实践',
                        name: '余方玉',
                        post: '阿里巴巴达摩院 内核技术专家',
                      }
                    ]
                  },
                  {
                    time: '14:25-14:45',
                    content: [
                      {
                        title: 'RISC-V 机密计算技术探索',
                        name: '张健',
                        post: '北京开源芯片研究院 产品经理',
                      }
                    ]
                  },
                  {
                    time: '14:45-15:05',
                    content: [
                      {
                        title: 'openEuler x 算能SRA3服务器，RISC-V 架构落地数据中心',
                        name: '魏超',
                        post: '北京算能科技有限公司 嵌入式软件工程师',
                      }
                    ]
                  },
                ],
              }
            ]
          },
          {
            title: 'SIG Gathering',
            children: [],
            tab: [],
            list: [
              {
                title: 'SIG Gathering',
                children: [
                  {
                    label: 'Track ：Kernel & 基础服务',
                    time: '16:00 - 17:30',
                    address: '1层-105',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-Kernel',
                    children: [],
                  },
                  {
                    label: 'Track ：开发者使能与工具',
                    time: '16:00 - 17:30',
                    address: '1层-106',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-community%26developer',
                    children: [],
                  },
                  {
                    label: 'Track ：AI使能与应用',
                    time: '16:00 - 17:30',
                    address: '1层-103',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-ai',
                    children: [],
                  },
                  {
                    label: 'Track ：灵衢UnifiedBus',
                    time: '16:00 - 17:30',
                    address: 'G层-G02',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-UnifiedBus',
                    children: [],
                  },
                  {
                    label: 'Track ：Embedded & ROS',
                    time: '16:00 - 17:30',
                    address: 'G层-G10A',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-Embedded-ROS',
                    children: [],
                  },
                  {
                    label: 'Track ：RISC-V',
                    time: '16:00 - 17:30',
                    address: 'G层-G10B',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-RISC-V',
                    children: [],
                  },
                  {
                    label: 'Track ：安全与机密计算',
                    time: '16:00 - 17:30',
                    address: 'G层-G01A',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-gathering-ICC-NSG',
                    children: [],
                  },
                ]
              },
              {
                title: '开发者之夜',
                children: [
                  {
                    label: '开发者之夜',
                    time: '18:00 - 20:00',
                    address: '畅春厅',
                    children: [],
                  }
                ]
              }
            ]
          }
        ]
      }
    ]
  },
  guest: {
    title: '活动嘉宾',
    list: [
      {
        title: '操作系统大会主论坛嘉宾',
        children: [
          {
            name: '倪光南',
            img: niguangnan,
            title: '中国工程院\n院士',
          },
          {
            name: '程晓明',
            img: chengxiaoming,
            title: '开放原子开源基金会\n理事长',
          },
          {
            name: '杨超斌',
            img: yangchaobin,
            title: '华为公司董事、ICT BG\nCEO',
          },
          {
            name: '邵广禄',
            img: shaoguanglu,
            title: '中国电信集团科技委主任\n中国电信集团原总经理',
          },
          {
            name: '熊伟',
            img: xiongwei,
            title: '开放原子开源基金会\nopenEuler委员会主席',
          },
          {
            name: '杜海',
            img: duhai,
            title: '百度智能云混合云部\n总经理',
          },
          {
            name: '曾文斌',
            img: zengwenbin,
            title: '美团操作系统\n总监',
          },
          {
            name: '杨明明',
            img: yangmingming,
            title: '菲尼克斯中国公司\n副总裁',
          },
        ]
      },
      {
        title: 'openEuler Summit 2025主论坛嘉宾',
        children: [
          {
            name: '彭红华',
            img: penghonghua,
            title: '华为ICT战略与业务发展部\n总裁',
          },
          {
            name: '熊伟',
            img: xiongwei,
            title: '开放原子开源基金会\nopenEuler委员会主席',
          },
          {
            name: '唐晓蕾',
            img: tangxiaolei,
            title: 'AMD公司全球\n副总裁',
          },
          {
            name: 'Andrew Wafaa',
            img: andrew,
            title: 'Arm Senior Director Software Communities & Fellow\nPyTorch Board',
          },
          {
            name: '胡欣蔚',
            img: huxinwei,
            title: 'openEuler技术委员会\n主席',
          },
          {
            name: '胡春明',
            img: huchunming,
            title: '北京航空航天大学软件学院\n院长',
          },
          {
            name: 'Grant Likely',
            img: grant,
            title: 'CTO\nLinaro',
          },
          {
            name: '林振福',
            img: linzhenfu,
            title: '南网数研院\n技术专家',
          },
          {
            name: '高立江',
            img: gaolijiang,
            title: '中邮信息科技（北京）\n云原生平台负责人',
          },
          {
            name: '叶青龙',
            img: yeqinglong,
            title: '麒麟信安(广东)科技有限公司\n研发总监',
          },
          {
            name: '陈友宣',
            img: chenyouxuan,
            title: '广州汽车集团股份有限公司\nHPC负责人',
          },
        ]
      },
      {
        title: '分论坛演讲嘉宾',
        children: [
          {
            name: '杜冬冬',
            img: dudongdong,
            title: '上海交通大学\n助理教授',
          },
          {
            name: '卢俊',
            img: lujun,
            title: '中国科学院广州生物医药与健康研究院\n正高级工程师\n谱系设施工程技术中心副主任',
          },
          {
            name: '张涛',
            img: zhangtao,
            title: '江苏润和软件股份有限公司\n嵌入式部门经理',
          },
          {
            name: '姜浩然',
            img: jianghaoran,
            title: '麒麟软件有限公司\n内核高级研发工程师',
          },
          {
            name: '段程浩',
            img: duanchenghao,
            title: '麒麟软件有限公司\n内核高级研发工程师',
          },
          {
            name: '唐友灵',
            img: tangyouling,
            title: '麒麟软件有限公司\n内核高级研发工程师',
          },
          {
            name: '宋师',
            img: songshi,
            title: '广州数控设备有限公司\n副总工程师',
          },
          {
            name: '姚广忠',
            img: yaoguangzhong,
            title: '河南昆仑技术有限公司\n技术专家',
          },
          {
            name: '杨露',
            img: yanglu,
            title: '菲尼克斯电气\nPLCnext Software Application 总监',
          },
          {
            name: '张锐',
            img: zhangrui,
            title: '英特尔亚太研发有限公司\n高级软件工程师',
          },
          {
            name: '王文宽',
            img: wangwenkuan,
            title: '超威半导体产品(中国)有限公司\nAMD Linux 内核专家',
          },
          {
            name: 'Cryolitia',
            img: cryolitia,
            title: '统信软件\n内核研发工程师',
          },
          {
            name: '宋家雨',
            img: songjiayu,
            title: 'cxl技术应用俱乐部\n秘书长',
          },
          {
            name: '靳晓旭',
            img: jinxiaoxu,
            title: '用友\n海外中企出海业务总监',
          },
          {
            name: '张骁',
            img: zhangxiao,
            title: '讯飞星火\nAI工程院智算平台总监',
          },
          {
            name: '张泽华',
            img: zhangzehua,
            title: '京东\n算法总监',
          },
          {
            name: '孙政华',
            img: sunzhenghua, 
            title: '深信服\n中央平台部研发主管',
          },
          {
            name: '周强',
            img: zhouqiang,
            title: '风河\n高级交付总监',
          },
          {
            name: '刘秉伟',
            img: liubingwei,
            title: '英特尔（中国）\n软件技术事业部高级总监',
          },
          {
            name: '蔡鑫奇',
            img: caixinqi,
            title: '北京航天微系统与信息技术研究所',
          },
          {
            name: '曾逸',
            img: zengyi,
            title: '广东拓斯达科技股份有限公司\n机器人市场总监',
          },
          {
            name: '丁嘉辉',
            img: dingjiahui,
            title: '华为技术有限公司\n高级软件工程师',
          },
          {
            name: '费建强',
            img: feijianqiang,
            title: '华为\n云原生专家',
          },
          {
            name: '胡旷',
            img: hukuang,
            title: '智元机器人\n生态总监',
          },
          {
            name: '江东平',
            img: jiangdongping,
            title: '南京启诺信息技术有限公司\n总监',
          },
          {
            name: '金龙',
            img: jinlong,
            title: '宝德计算机系统股份有限公司\n解决方案销售总监',
          },
          {
            name: '李朝阳',
            img: lichaoyang,
            title: '华中科技大学开放原子俱乐部\n学生',
          },
          {
            name: '李鹏',
            img: lipeng,
            title: '中国科学院软件研究所\n副研究员',
          },
          {
            name: '梁义',
            img: liangyi,
            title: '华为\n通用Serverless首席专家',
          },
          {
            name: '吴健',
            img: wujian,
            title: '鼎桥技术有限公司\n机器人解决方案首席架构师',
          },
          {
            name: '刘保玉',
            img: liubaoyu,
            title: '粤港澳大湾区国家技术创新中心\n嵌入式数据库总经理',
          },
          {
            name: '鲁卫军',
            img: luweijun,
            title: '华为技术有限公司\n高级软件工程师',
          },
          {
            name: '谭睿',
            img: tanrui,
            title: '北京光亚鸿道操作系统有限公司\n产品经理',
          },
          {
            name: '万峰',
            img: wanfeng,
            title: '麒麟软件有限公司\n高级软件工程师',
          },
          {
            name: '王大维',
            img: wangdawei,
            title: '成都菁蓉联创科技有限公司\n技术总工',
          },
          {
            name: '王伟',
            img: wangwei,
            title: '成都菁蓉联创科技有限公司\n无人装备事业部部长',
          },
          {
            name: '王晓云',
            img: wangxiaoyun,
            title: '中国科学院软件研究所\n机器人系统工程师',
          },
          {
            name: '王志强',
            img: wangzhiqiang,
            title: 'CODESYS软件集团(中国)\n市场负责人',
          },
          {
            name: '魏建刚',
            img: weijiangang,
            title: '润和软件股份有限公司\nOS资深专家',
          },
          {
            name: '吴衡',
            img: wuheng,
            title: '天翼云科技有限公司\n研发工程师',
          },
          {
            name: '吴治国',
            img: wuzhiguo,
            title: 'Linaro\nEngineer',
          },
          {
            name: '徐磊',
            img: xulei,
            title: '超聚变数字技术有限公司\n高级研发工程师',
          },
          {
            name: '徐佩文',
            img: xupeiwen,
            title: '南京菲尼克斯电气\nPLCnext产品经理',
          },
          {
            name: '杨彬',
            img: yangbin,
            title: '软通动力信息技术（集团）股份有限公司\n开源欧拉研究中心主任',
          },
          {
            name: '袁礼鹏',
            img: yunlipeng,
            title: '华为技术有限公司\n高级软件工程师',
          },
          {
            name: '翟文杰',
            img: zhuowenjie,
            title: '华为技术有限公司\n高级软件工程师',
          },
          {
            name: '钟茂恒',
            img: zhongmaoheng,
            title: '北京凝思软件股份有限公司\nManager',
          },
          {
            name: '钟涛',
            img: zhongtao,
            title: '华为技术有限公司\n工程师',
          },
          {
            name: '刘新良',
            img: liuxinliang,
            title: 'Linaro Limited\nEngineer',
          },
          {
            name: 'Andy',
            img: andy,
            title: '京东\n安全工程师',
          },
          {
            name: '贺礼云',
            img: heliyun,
            title: '凝思软件成都研发中心\n副总经理',
          },
          {
            name: '李萍',
            img: liping,
            title: '华为技术有限公司\n高级软件工程师',
          },
          {
            name: '王麟',
            img: wanglin,
            title: '联通数字科技有限公司\n高级研发专家',
          },
          {
            name: '王宇诚',
            img: wangyucheng,
            title: '湖南麒麟信安科技股份有限公司\n研发工程师',
          },
          {
            name: '武成岗',
            img: wuchenggang,
            title: '中国科学院计算技术研究所\n研究员',
          },
          {
            name: '辛晓慧',
            img: xinxiaohui,
            title: 'Linaro\n软件工程总监',
          },
          {
            name: '张驰',
            img: zhangchi,
            title: '三未信安科技股份有限公司\n技术总监',
          },
          {
            name: '冯焱',
            img: fengyan,
            title: '北京惠每云科技有限公司\n高级后端工程师',
          },
          {
            name: '韩远坤',
            img: hanyuankun,
            title: '华为技术有限公司\n华为AI通信专家',
          },
          {
            name: '黄俊',
            img: huangjun,
            title: '招商证券股份有限公司\n核心系统架构师',
          },
          {
            name: '李静静',
            img: lijingjing,
            title: '北京东方通科技有限公司\n产品经理',
          },
          {
            name: '李宇振',
            img: liyuzhen,
            title: '华为技术有限公司\nAI工程师',
          },
          {
            name: '孙亚楠',
            img: sunyanan,
            title: '河南昆仑技术有限公司\n高级工程师',
          },
          {
            name: '郑晨卉',
            img: zhegnchenhui,
            title: '华为技术有限公司\n软件工程师',
          },
        ]
      },
      {
        title: '技术分论坛出品人',
        children: [
          {
            name: '谢秀奇',
            img: xiexiuqi,
            title: 'openEuler\nKernel Commiter',
          },
          {
            name: '罗钰凯',
            img: luoyukai,
            title: 'openEuler 安全委员会\n委员',
          },
          {
            name: '杜开田',
            img: dukaitian,
            title: 'openEuler\n兼容性SIG组Maintainer',
          },
          {
            name: '李晨斌',
            img: lichenbin,
            title: '北京凝思软件股份有限公司市场产品专家\nopenEuler用户委员会委员',
          },
          {
            name: '唐葛亮',
            img: tanggeliang,
            title: '麒麟软件有限公司\nopenEuler 技术委员会',
          },
          {
            name: '牛涛',
            img: zhutao,
            title: 'openEuler\nsig-UnifiedBus Maintainer',
          },
          {
            name: '张以正',
            img: zhangyizheng,
            title: 'openEuler\nsig-Intelligence Commiter',
          },
          {
            name: '陈鑫',
            img: chenxin,
            title: 'openEuler\nsig-embedded',
          },
          {
            name: '朱泽旭',
            img: zhuzexu,
            title: 'openEuler\nDevStation SIG及兼容性 SIG Maintainer',
          },
          {
            name: '任慰',
            img: renwei,
            title: 'openEuler技术委员会\n委员',
          },
          {   
            name: '王经纬',
            img: wangjingwei,
            title: '中国科学院软件研究所\nopenEuler TC',
          },
          {
            name: '李洋',
            img: liyang,
            title: 'openEuler社区\nsig-分布式中间件maintainer',
          }
        ]
      }
    ]
  },
  partner: {
    title: '共建单位',
    list: [
      {
        title: '主办方',
        logos: [
          {
            light: openEuler,
            dark: openEulerDark
          }
        ]
      },
      {
        title: '联合主办方',
        logos: [
          {
            light: huawei,
            dark: huawei_dark
          },
          {
            light: qiling,
            dark: qiling_dark
          },
          {
            light: xinan,
            dark: xinan_dark
          }
        ]
      },
      {
        title: '协办方',
        logos: [
          {
            light: tongxin,
            dark: tongxin_dark
          },
          {
            light: ningsi,
            dark: ningsi_dark
          },
          {
            light: xfusion,
            dark: xfusion_dark
          },
          {
            light: runhe,
            dark: runhe_dark
          },
          {
            light: ruantong,
            dark: ruantong_dark
          },
          {
            light: dongfang,
            dark: dongfang_dark
          },
        ]
      },
      {
        title: '支持方',
        logos: [
          {
            light: baolande,
            dark: baolande_dark
          },
        ]
      }
    ]
  },
  previous: {
    title: '精彩回顾',
    list: [
      {
        name: 'openEuler Developer Day 2025',
        link: '/zh/interaction/summit-list/devday2025/',
      },
      {
        name: 'openEuler Summit 2024',
        link: '/zh/interaction/summit-list/summit2024/',
      },
      {
        name: 'openEuler SIG Gathering 2024',
        link: '/zh/interaction/summit-list/sig-gathering-2024/',
      },
      {
        name: 'openEuler Summit 2023',
        link: '/zh/interaction/summit-list/summit2023/',
      },
      {
        name: 'openEuler Developer Day 2023',
        link: '/zh/interaction/summit-list/devday2023/',
      },
      {
        name: 'openEuler Summit 2022',
        link: '/zh/interaction/summit-list/summit2022/',
      },
    ],
  },
};
