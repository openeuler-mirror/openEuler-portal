...
import roleDescriptionContent from '#content/sig/role-description';
...
const content = computed(() => roleDescriptionContent[locale.value]);
const communityMember = computed(() => content.value.community_member);
const sections = computed(() => [
  content.value.contributor,
  content.value.committer,
  content.value.maintainer,
]);
...
const getSectionBg = (item: any) => {
  if (lePadV.value) {
    return isDark.value ? item.bg_mb_dark : item.bg_mb;
  } else {
    return isDark.value ? item.bg_dark : item.bg;
  }
};
